...
const HiscoresDashboard = () => {
  ...
  const fetchData = async (filter: TimeFilter) => {
    try {
      setIsLoading(true);
      const response = await fetch(`http://localhost:8080/dabData?filter=${encodeURIComponent(filter)}`);
      ...
      const activityResponse = await fetch('http://localhost:8080/dabs/recentactivity');
